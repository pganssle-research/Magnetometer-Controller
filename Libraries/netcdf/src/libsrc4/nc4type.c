...
$Id: nc4type.c,v 1.68 2010/01/11 19:27:12 ed Exp $
*/
		  ...
		  nc_type typeid2, int *equal)
{
   ...
   if (equal)
      *equal = (int)H5Tequal(type1->native_typeid, type2->native_typeid);
...
int
nc_insert_compound(int ncid, nc_type typeid, const char *name, size_t offset, 
...
int
nc_inq_compound_fieldindex(int ncid, nc_type typeid, const char *name, int *fieldidp)
{
   NC_FILE_INFO_T *nc;
   NC_TYPE_INFO_T *type;
   NC_FIELD_INFO_T *field;
...
int
nc_def_opaque(int ncid, size_t datum_size, const char *name, 
	      nc_type *typeidp)
{
...
int
nc_def_vlen(int ncid, const char *name, nc_type base_typeid, 
	    nc_type *typeidp)
{
...
int
nc_put_vlen_element(int ncid, int typeid, void *vlen_element, 
...
/* Insert one element into an already allocated vlen array element. */
int
nc_get_vlen_element(int ncid, int typeid, const void *vlen_element, 
